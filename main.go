...
func main() {
	...
	if cert != "" && key != "" {
		go func() {
			log.Infof("Start to listening the incoming request on https address: %s", viper.GetString("tls.port"))
			log.Info(http.ListenAndServeTLS(viper.GetString("tls.port"), cert, key, g).Error())
		}()
	}
